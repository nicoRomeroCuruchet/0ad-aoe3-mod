...
def test_random_policy_is_reproducible_for_a_multidiscrete_space():
...
def test_team_oracle_emits_assignment_actions_for_empty_and_busy_villagers():
    from rl.agents.baselines import TeamGatherOraclePolicy
    from rl.gather.observation import (
        TeamObservationScales,
        TeamSnapshot,
    ...
    snapshot = TeamSnapshot(
        ...
        resource_xz=((300.0, 100.0), (400.0, 100.0)),
        resource_remaining=(200.0, 200.0),
        carried=(0.0, 0.0, 20.0, 0.0),
        target_index=(0, 1, 0, 1),
        gather_cycle_active=(False, False, False, True),
        dropsite_xz=(50.0, 150.0),
        stock=0.0,
    )
    observation = build_team_observation(
    ...
    policy = TeamGatherOraclePolicy(4, 2, action_mode="assignment_click")

    action = policy.act(observation, deterministic=True)

    # Both trees are already claimed by busy villagers, so the idle villagers
    # wait. The active (but not carrying) worker safely reissues its own gather
    # order; this prevents a stalled UnitAI cycle from stranding a finite tree.
    np.testing.assert_array_equal(action, np.array([0, 0, 0, 2], dtype=np.int64))
    assert action.shape == (4,)
    assert action.dtype == np.dtype(np.int64)


def test_team_oracle_does_not_claim_a_busy_villagers_current_tree():
    from rl.agents.baselines import TeamGatherOraclePolicy
    from rl.gather.observation import (
        TeamObservationScales,
        TeamSnapshot,
    ...
    snapshot = TeamSnapshot(
        villager_xz=((295.0, 100.0), (290.0, 100.0)),
        resource_xz=((300.0, 100.0), (400.0, 100.0)),
        resource_remaining=(200.0, 200.0),
        carried=(0.0, 0.0),
        target_index=(0, None),
        gather_cycle_active=(True, False),
        dropsite_xz=(50.0, 150.0),
        stock=0.0,
    )
    observation = build_team_observation(
    ...
    action = TeamGatherOraclePolicy(
        2,
        2,
        action_mode="assignment_click",
    ).act(observation, deterministic=True)

    # Worker 0 refreshes its own live target; worker 1 takes the only other
    # tree, so the oracle still never issues a duplicate claim.
    np.testing.assert_array_equal(action, np.array([1, 2], dtype=np.int64))


def test_team_oracle_does_not_assign_a_depleted_tree():
    from rl.agents.baselines import TeamGatherOraclePolicy
    from rl.gather.observation import (
        TeamObservationScales,
        TeamSnapshot,
    ...
    snapshot = TeamSnapshot(
        villager_xz=((295.0, 100.0),),
        resource_xz=((300.0, 100.0), (400.0, 100.0)),
        resource_remaining=(0.0, 200.0),
        carried=(0.0,),
        target_index=(None,),
    ...
    np.testing.assert_array_equal(action, np.array([2], dtype=np.int64))
